...
echo "Run 015C - Full eval removed images, region_mask_mode=removed"
date
hostname
which python
python -V
...
OUTDIR="outputs/eval_removed_random_removed"
...
../../.venv_epistemic/bin/python eval_scripts/eval_caption.py \
  ...
  --output_dir ../../outputs/eval_removed_random_removed \
  --num_samples 522 \
  --max_new_tokens 64 \
  --use_ours \
  --region_mask_mode random_removed \
  --token_region_root ../../hpc_inputs/token_regions

cd /nethome/ptasathish/projects/region-vlm-uncertainty

echo "Checking outputs"
find outputs/eval_removed_random_removed -maxdepth 3 -type f | sort | head -20
wc -l outputs/eval_removed_random_removed/captions.jsonl
find outputs/eval_removed_random_removed/region_uncertainty -maxdepth 1 -type f -name "*.json" | wc -l

echo "Status: Run 015C full eval removed none completed."
date
