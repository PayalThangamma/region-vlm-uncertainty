...
echo "Run 015D - Full eval removed images, region_mask_mode=context"
date
hostname
which python
python -V
...
OUTDIR="outputs/eval_removed_random_context"
...
echo "Running full eval context-region masking"
../../.venv_epistemic/bin/python eval_scripts/eval_caption.py \
  ...
  --output_dir ../../outputs/eval_removed_random_context \
  --num_samples 522 \
  --max_new_tokens 64 \
  --use_ours \
  --region_mask_mode random_context \
  --token_region_root ../../hpc_inputs/token_regions

cd /nethome/ptasathish/projects/region-vlm-uncertainty

echo "Checking outputs"
find outputs/eval_removed_random_context -maxdepth 3 -type f | sort | head -20
wc -l outputs/eval_removed_random_context/captions.jsonl
find outputs/eval_removed_random_context/region_uncertainty -maxdepth 1 -type f -name "*.json" | wc -l

echo "Status: Run 015D full eval removed random_context completed."
date
