...
echo "Run 015E - Full eval removed images, region_mask_mode=background"
date
hostname
which python
python -V
...
OUTDIR="outputs/eval_removed_random_background"
...
echo "Running full eval background-region masking"
../../.venv_epistemic/bin/python eval_scripts/eval_caption.py \
  ...
  --output_dir ../../outputs/eval_removed_random_background \
  --num_samples 522 \
  --max_new_tokens 64 \
  --use_ours \
  --region_mask_mode random_background \
  --token_region_root ../../hpc_inputs/token_regions

cd /nethome/ptasathish/projects/region-vlm-uncertainty

echo "Checking outputs"
find outputs/eval_removed_random_background -maxdepth 3 -type f | sort | head -20
wc -l outputs/eval_removed_random_background/captions.jsonl
find outputs/eval_removed_random_background/region_uncertainty -maxdepth 1 -type f -name "*.json" | wc -l

echo "Status: Run 015E full eval removed random_background completed."
date
