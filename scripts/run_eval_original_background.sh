...
OUTDIR="outputs/eval_original_background"
...
../../.venv_epistemic/bin/python eval_scripts/eval_caption.py \
  ...
  --output_dir ../../outputs/eval_original_background \
  --num_samples 522 \
  --max_new_tokens 64 \
  --use_ours \
...
find outputs/eval_original_background -maxdepth 3 -type f | sort | head -20
wc -l outputs/eval_original_background/captions.jsonl
find outputs/eval_original_background/region_uncertainty -maxdepth 1 -type f -name "*.json" | wc -l

echo "Status: Run 015E full eval removed background completed."
date
